...
router.post('/', function(req, res, next) {
    ...
    if (global.print_log) {
        console.log('user simple query req body: ' + JSON.stringify(req_body));
    }
    ...
    // 获取用户
    var strSqlUser = "SELECT id,name,status,remark FROM res_user";
    ...
    if (all == undefined || all == null || all <= 0) {
        if (strCondition.length > 0) {
            strCondition += " AND weight>0";
        }
        else {
            strCondition +=  " WHERE weight>0";
        }
    }
