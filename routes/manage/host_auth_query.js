/**
 * Created by fushou on 2019/3/12.
 */
/**
...
router.post('/', function(req, res, next) {
    ...
    if (global.print_log) {
        console.log('host auth query req body: ' + JSON.stringify(req_body));
    }
    ...
    var strMaxSql = "SELECT t1.user_id,t2.name AS user_name FROM res_as_auth t1, res_user t2 ";
    strMaxSql += "WHERE t1.user_id=t2.id ";
    // 获取记录
    // SELECT t1.user_id,t2.name AS user_name, COUNT(*) AS host_count FROM res_host_auth t1, res_user t2 WHERE t1.user_id=t2.id GROUP BY t1.user_id
    var strSql = "SELECT t1.user_id,t2.name AS user_name,COUNT(*) AS host_count FROM ";
    ...
    db.query(strMaxSql, function(err, result, fields){
        ...
        db.query(strSql, function(err2, result2, field2){
