...
router.post('/', function(req, res, next) {
    ...
    if (global.print_log) {
        console.log('user group delete req body: ' + JSON.stringify(req_body));
    }
    ...
    if (group_id < 1) {
        ...
        var strSql = "DELETE FROM res_user_group WHERE id=" + group_id;

        var strUserSql = "UPDATE res_user SET group_id=0 WHERE group_id=" + group_id;
        ...
        modify_db.delete(strSql, function(err, result) {
            ...
            if (err) {
                ...
                // 更新用户表信息
                modify_db.modify(strUserSql, null, function(err2, result2){
                    if (global.print_log) {
                        console.log('reqSql: ' + strUserSql);
